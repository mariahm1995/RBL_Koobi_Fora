#' Run an Ecometric Model with Independent Binning for Mean and SD Traits
#'
#' Models the relationship between community trait distributions and the environment
#' by estimating environmental values in binned ecometric trait space.
#'
...
#' @param transform_fun Optional transformation function (e.g., \code{log(x + 1)}).
#' @param inv_transform_fun Optional inverse transformation (e.g., \code{exp(x) - 1}).
...
#'
#' @return A list with model results, spatial grid, diagnostics, and settings.
#' @export
